...
public class SiparisController {
	...
	public ResponseEntity<SiparisSchema> SiparisKaydet(@RequestBody(required = true) @Valid SiparisSchema siparisSchema,
			UrunSchema urunSchema) {
		try {
			SiparisSchema _sSchema = siparisRepository
					.save(new SiparisSchema(siparisSchema.getMasaNo(),
							siparisSchema.getUrun(urunSchema.getUrun(), urunSchema.getFiyat(),
									urunSchema.getIcindekiler()),
							siparisSchema.getEkstralar(), siparisSchema.getNot(), siparisSchema.getToplamFiyat(),
							siparisSchema.isSiparisAktifMi()));
